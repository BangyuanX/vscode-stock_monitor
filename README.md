...
> 美股的盘前（美东 4:00-9:30）和盘后（美东 16:00-20:00）数据自动获取，无需额外配置。
> 在 `stock-bar.format` 中加入 `${session}` 即可在状态栏看到 🌅/🌙 标记。
...
| `${session}${icon}${name} ${price}` | 🌅📉AAPL 149.80（盘前） |
