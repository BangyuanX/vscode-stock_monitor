...
点击侧栏标题栏的公文包按钮，或执行 `Stock Bar: 管理 A 股持仓`：

1. 在当前 A 股自选列表中填写持仓股数，清空或填 `0` 表示不计入。
2. 表格即时预览单项与合计今日盈亏，点击“保存持仓”生效。
3. 今日盈亏按 `持仓股数 ×（现价 − 昨收价）` 计算，只统计 A 股。
4. 配置持仓后，汇总项固定显示在状态栏所有行情标的之前，点击它可重新打开持仓管理。

尚未开市或停牌时，如果行情源返回最新价 `0`、但昨收有效，插件会暂按昨收显示并将当日涨跌及持仓今日盈亏记为 `0`；悬停详情会标明当前使用了昨收回退价。

### 在多台电脑间同步

Stock Bar 使用 VS Code 的扩展专用同步状态保存全部配置，包括自选代码及顺序、状态栏图钉、A 股持仓、小数位数、显示乘数、刷新间隔、格式和颜色。数据不会依赖手工维护一份 `settings.json`。
...
| `Stock Bar: 管理 A 股持仓` | 可视化维护持仓股数与查看今日盈亏 |
...
| `Stock Bar: 管理 A 股持仓` | 维护 A 股持仓并查看今日盈亏 |
